...
float PID(float input)
{
    ...
    output = (KP_now * error) + (KI_now * Ierr) + (KD_now * Derr);
...
void baca_sensor() {
    current_ADC= getADCAvg(ADC_CHANNEL_2, 30, &hadc1);
    //current_A=(0.0139*current_ADC) -28.56-0.1;
	current_A= adcToVolt(current_ADC-2015.0)*10.0;

    voltage_ADC= getADCAvg(ADC_CHANNEL_0, 30, &hadc1);
